...
bash.runCommand "${pname}-${version}-x86_64"
  {
    ...
    meta = {
      description = "Tools for manipulating binaries (linker, assembler, etc.) for x86_64";
  ...
  ''
    ...
    # Build CC wrapper (for build programs that need to run on i386)
    ...
    # Cross CC wrapper (produces x86_64 binaries)
    cat > gcc <<EOF
    #!${bash}/bin/bash
    exec ${crossGcc}/bin/${target}-gcc \
      ...
      -L ${zlib}/lib \
      -Wl,--dynamic-linker=${crossGlibc}/lib/ld-linux-x86-64.so.2 \
      -Wl,-rpath,${zlib}/lib \
      -Wl,-rpath,${crossGlibc}/lib \
      "\$@"
    EOF
    chmod 555 gcc
    ...
    export PATH="''${PWD}:${buildBinutils}/bin:$PATH"
    ...
    (
      ...
      /build/wrappers/build-cc -o crcgen crcgen.c
    ...
    (
      ...
      CONFIG_SHELL=${bash}/bin/bash \
        SHELL=${bash}/bin/bash \
        CC=/build/wrappers/build-cc \
        CXX=/build/wrappers/build-cxx \
        CPP="/build/wrappers/build-cc -E" \
        CXXCPP="/build/wrappers/build-cxx -E" \
        CPPFLAGS="-I${buildZlib}/include" \
        LDFLAGS="-L${buildZlib}/lib" \
        ./configure \
        --build=${buildTarget} \
        --host=${buildTarget} \
        --target=${target}
      ${gnumake}/bin/make SHELL=${bash}/bin/bash headers
      mv configure.ac.bak configure.ac
      ${gnumake}/bin/make SHELL=${bash}/bin/bash distclean
    )
    ...
    (
      ...
      CONFIG_SHELL=${bash}/bin/bash \
        SHELL=${bash}/bin/bash \
        CC=/build/wrappers/build-cc ./configure --enable-maintainer-mode
      ${gnumake}/bin/make SHELL=${bash}/bin/bash maint-deps
      ${gnumake}/bin/make SHELL=${bash}/bin/bash distclean
    )
    ...
    # Install
    ${gnumake}/bin/make -j "$NIX_BUILD_CORES" SHELL=${bash}/bin/bash MAKEINFO=true install
