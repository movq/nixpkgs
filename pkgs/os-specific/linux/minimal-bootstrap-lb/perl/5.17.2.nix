...
let
  ...
  src = fetchurl {
    ...
    hash = "sha256-pHxZ/TZnARIFvk04T02i3rCnVEvfHMLvpm6f7+pDVXY=";
  };
...
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    ${gnupatch}/bin/patch -Np0 -i ${./5.17.2/patches/0001-Revert-regen-mk_invlists.pl-Fail-if-inversion-list-n.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.17.2/patches/0001-Unicode-UCD-prop_invlist-Allow-to-return-internal-pr.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.17.2/patches/Sort-files-returned-from-all_files_in_dir-for-consis.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.17.2/patches/a2p-c-bison.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.17.2/patches/reproducible-mktables.patch}
    ...
    cp ${./5.17.2/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.17.2/files/Compress-Raw-Zlib_config.in} \
