...
let
  ...
  src = fetchurl {
    ...
    hash = "sha256-RaIo2u9m0C/czIIOcfh+QNjj3x/EQx+NRYDsCAM4Zr0=";
  };

  metaconfigSrc = fetchurl {
    url = "https://github.com/Perl/metaconfig/archive/5.36.0.tar.gz";
    hash = "sha256-B2fwVmBnyf1dF/X/U/oEucRaGCr6GJ3AW1+EGNWIjZI=";
  };
in
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    ${gnupatch}/bin/patch -Np0 -i ${./5.36.3/patches/Devel-PPPort-remove-sprintf.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.36.3/patches/PPPort-pm-reproducible.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.36.3/patches/Revert-mktables-Use-builtin-refaddr.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.36.3/patches/remove-machine-generated.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.36.3/patches/reproducible-mktables.patch}
    ...
    cp ${./5.36.3/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.36.3/files/Compress-Raw-Zlib_config.in} \
    ...
    PERL5LIB="''${tokenizerPath}" ${oldPerl}/bin/perl -Ilib regen/mk_invlists.pl
