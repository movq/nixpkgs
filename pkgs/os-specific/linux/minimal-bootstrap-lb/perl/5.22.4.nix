...
let
  ...
  src = fetchurl {
    ...
    hash = "sha256-cTJD3OJ9eqC9v1KyBw3lzkSfn/vMFKk++8byvv8PXOg=";
  };

  unicodeSrc = fetchurl {
    url = "https://www.cpan.org/src/5.0/perl-5.24.4.tar.xz";
    hash = "sha256-fwgCh/9kdQJwaJhDrpRfAhWaM8uPL8kQJIwVvvul24Q=";
  };

  metaconfigSrc = fetchurl {
    url = "https://github.com/Perl/metaconfig/archive/5.26.1.tar.gz";
    hash = "sha256-RkH1EnpVzy4ej5JT7Cf7oQtbdwdrQWPn9L3H9nDRqvA=";
  };
in
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    cp ${unicodeSrc} unicode.tar.xz
    ${xz}/bin/xz -d -f unicode.tar.xz
    ${gnutar}/bin/tar xf unicode.tar
    rm unicode.tar
    ...
    mv perl-5.24.4/lib/unicore/{version,mktables,*.txt} perl-5.22.4/lib/unicore/
    mv perl-5.24.4/lib/unicore/auxiliary/*.txt perl-5.22.4/lib/unicore/auxiliary/
    mv perl-5.24.4/lib/unicore/extracted/*.txt perl-5.22.4/lib/unicore/extracted/

    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/Allow-strictly-named-map-tables.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/Make-mktables-work-on-5.22.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/PPPort-pm-reproducible.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/Sort-files-returned-from-all_files_in_dir-for-consis.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/regcharclass-remove-experimental.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.22.4/patches/reproducible-mktables.patch}
    ...
    cp ${./5.22.4/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.22.4/files/Compress-Raw-Zlib_config.in} \
    ...
    rm cpan/Unicode-Collate/Collate/keys.txt cpan/Devel-PPPort/t/*.t
    ...
    ${gnused}/bin/sed -i \
      -e "s:/pro/3gl/CPAN/lib/dist:${dist}/lib/perl5/5.6.2:" \
      -e "s:/pro/3gl/CPAN/perl:''${PWD}:" \
      ../metaconfig*/U/mkglossary
    ${oldPerl}/bin/perl ../metaconfig*/U/mkglossary > Porting/Glossary
