...
let
  ...
  src = fetchurl {
    ...
    hash = "sha256-H7TSe3XNJE6EnyUzICYO/hdQZBqv9KGM4NZ1Vv8blqU=";
  };
...
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0001-Unicode-UCD-Allow-internal-properties-in-invmap.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0002-Unicode-UCD-Add-prop_values-function.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0003-Refactor-ExtUtils-Embed-to-work-with-miniperl.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0004-Tweak-ExtUtils-Embed-s-generated-C-code-to-be-closer.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0005-Move-an-inversion-list-generation-to-mktables.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/0006-Partial-backport-Work-properly-under-UTF-8-LC_CTYPE-.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/Encode-reproducible-includes.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/Generate-Errno-and-Pod-Functions-with-deterministic-.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/PPPort-pm-reproducible.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/ParseXS-reproducible.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/Revert-regen-mk_PL_charclass-pl.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/Sort-files-returned-from-all_files_in_dir-for-consis.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/Unicode-UCD-search_invlist.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/a2p-c-bison.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.18.4/patches/reproducible-mktables.patch}
    ...
    cp ${./5.18.4/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.18.4/files/Compress-Raw-Zlib_config.in} \
    ...
    ${gnused}/bin/sed -i \
      "s/_Perl_Multi_Char_Folds_invlist/_Perl_Folds_To_Multi_Char_invlist/" \
    ...
    ${gnumake}/bin/make -j "$NIX_BUILD_CORES" miniperl lib/unicore/Name.pm
    ${gnupatch}/bin/patch -Np1 -R -i ${./5.18.4/patches/Revert-regen-mk_PL_charclass-pl.patch}
