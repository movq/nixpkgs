...
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    mv perl-5.30.3/lib/unicore/{version,mktables,*.txt} perl-5.24.4/lib/unicore/
    mv perl-5.30.3/lib/unicore/auxiliary/*.txt perl-5.24.4/lib/unicore/auxiliary/
    mv perl-5.30.3/lib/unicore/extracted/*.txt perl-5.24.4/lib/unicore/extracted/

    ${gnupatch}/bin/patch -Np0 -i ${./5.24.4/patches/PPPort-pm-reproducible.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.24.4/patches/Revert-regen-regcharclass.pl-avoid-autoderef-feature.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.24.4/patches/Sort-files-returned-from-all_files_in_dir-for-consis.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.24.4/patches/Unicode-UCD.pm-Add-undocumented-internal-feature.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.24.4/patches/reproducible-mktables.patch}
    ...
    cp ${./5.24.4/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.24.4/files/Compress-Raw-Zlib_config.in} \
