...
let
  ...
  src = fetchurl {
    ...
    hash = "sha256-Mx/MNW/M/EfobtCcIMCBhY+UZuQZMkAHp1eEQfhhF3w=";
  };
...
bash.runCommand "${pname}-${version}"
  ...
  ''
    ...
    ${gnupatch}/bin/patch -Np0 -i ${./5.15.7/patches/5.14_mk_PL_charclass.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.15.7/patches/Sort-files-returned-from-all_files_in_dir-for-consis.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.15.7/patches/a2p-c-bison.patch}
    ${gnupatch}/bin/patch -Np0 -i ${./5.15.7/patches/reproducible-mktables.patch}
    ...
    cp ${./5.15.7/files/config.over} config.over
    ...
    ${gnused}/bin/sed "s:%LIBDIR%:${zlib}/lib:" ${./5.15.7/files/Compress-Raw-Zlib_config.in} \
    ...
    rm -f ''${out}/*.0
    ${findutils}/bin/find ''${out}/lib/5.15.7 -name '*.pod' -delete
    ${findutils}/bin/find ''${out}/lib/5.15.7 -name '.packlist' -delete
  ''
