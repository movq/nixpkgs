...
bash.runCommand "${pname}-${version}-x86_64"
  ...
  ''
    ...
    CC_FOR_BUILD="''${PWD}/../gcc-for-build" \
      ...
      CC="${crossGcc}/bin/${target}-gcc --sysroot=${crossMusl} -Wl,--dynamic-linker=${crossMusl}/lib/ld-musl-x86_64.so.1 -Wl,-rpath,${zlib}/lib -Wl,-rpath,${crossMusl}/lib" \
      CXX="${crossGcc}/bin/${target}-g++ --sysroot=${crossMusl} -Wl,--dynamic-linker=${crossMusl}/lib/ld-musl-x86_64.so.1 -Wl,-rpath,${zlib}/lib -Wl,-rpath,${crossMusl}/lib" \
