...
bash.runCommand "${pname}-${version}-x86_64"
  {
    ...
    meta = {
      description = "GNU Compiler Collection 15.2.0 native x86_64 compiler";
  ...
  ''
    ...
    # Build CC wrappers (for programs that need to run on the build machine)
    ...
    export PATH="''${PWD}/build-tools:''${PWD}:${crossBinutils}/bin:${nativeBinutils}/bin:$PATH"
    export ACLOCAL_PATH="${libtool}/share/aclocal"
    export CPATH="${buildZlib}/include"
    export LIBRARY_PATH="${buildZlib}/lib"
    ...
    CC_FOR_BUILD="''${PWD}/../gcc-for-build" \
      CXX_FOR_BUILD="''${PWD}/../gxx-for-build" \
      CPPFLAGS_FOR_BUILD="-I${buildGmp}/include -I${buildMpfr}/include -I${buildMpc}/include -I${buildZlib}/include" \
      LDFLAGS_FOR_BUILD="-static -L${buildGmp}/lib -L${buildMpfr}/lib -L${buildMpc}/lib -L${buildZlib}/lib" \
      CC="${crossGcc}/bin/${target}-gcc --sysroot=${crossGlibc} -Wl,--dynamic-linker=${crossGlibc}/lib/ld-linux-x86-64.so.2 -Wl,-rpath,${zlib}/lib -Wl,-rpath,${crossGlibc}/lib" \
      CXX="${crossGcc}/bin/${target}-g++ --sysroot=${crossGlibc} -Wl,--dynamic-linker=${crossGlibc}/lib/ld-linux-x86-64.so.2 -Wl,-rpath,${zlib}/lib -Wl,-rpath,${crossGlibc}/lib" \
      CPPFLAGS="-I${gmp}/include -I${mpfr}/include -I${mpc}/include -I${zlib}/include" \
      LDFLAGS="-L${gmp}/lib -L${mpfr}/lib -L${mpc}/lib -L${zlib}/lib -Wl,-rpath,${zlib}/lib" \
      ../configure \
      --prefix=''${out} \
      --libdir=''${out}/lib \
      --build=${buildTarget} \
      ...
      --with-system-zlib \
      --with-sysroot=${crossGlibc} \
      --with-build-sysroot=${crossGlibc} \
    ...
    ${gnumake}/bin/make -j "$NIX_BUILD_CORES" \
      AR_FOR_TARGET=${nativeBinutils}/bin/ar \
      AS_FOR_TARGET=${nativeBinutils}/bin/as \
      LD_FOR_TARGET=${nativeBinutils}/bin/ld \
      NM_FOR_TARGET=${nativeBinutils}/bin/nm \
      RANLIB_FOR_TARGET=${nativeBinutils}/bin/ranlib \
