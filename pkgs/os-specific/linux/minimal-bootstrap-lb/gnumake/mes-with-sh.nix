...
bash.runCommand "${pname}-${version}"
  {
    ...
    meta = {
      description = "GNU Make 3.82 (mes libc, with sh)";
  ...
  ''
    ...
    # Patch /bin/sh
    replace --file job.c --output job.c \
      --match-on "char *default_shell = \"/bin/sh\";" \
      --replace-with "char *default_shell = \"${bash}/bin/sh\";"

    replace --file job.c --output job.c \
      --match-on "char default_shell[] = \"/bin/sh\";" \
      --replace-with "char default_shell[] = \"${bash}/bin/sh\";"

    tcc() { "${tinycc.compiler}/bin/tcc" -B "${tinycc.libs}/lib" "$@"; }
